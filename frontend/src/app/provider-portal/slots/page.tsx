...
export default function ProviderSlotsPage() {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (editForm.slotStatus !== editingSlot.slotStatus) dto.slotStatus = editForm.slotStatus as "available" | "blocked";
          ...
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            {modal === "recurring" && (
              <form onSubmit={handleRecurring} className="space-y-3">
                <div><label className="mb-1 block text-xs font-bold text-slate-600">Pattern</label><select value={recurForm.pattern} onChange={(e) => setRecurForm({ ...recurForm, pattern: e.target.value as "custom" | "daily" | "weekly" | "weekdays" | "weekends" })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none">
