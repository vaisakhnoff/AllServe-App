...
import { 
  Search, Star, Mic, ShieldCheck, Clock, CreditCard, 
  Headphones, ChevronRight, FileText, Users, CheckCircle2, Zap, ArrowRight
} from "lucide-react";
...
export default function HomePage() {
  ...
  const handleSearch = useCallback((query?: string) => {
    ...
    // Redirecting to login since public can't view providers fully without auth or we can just push to /providers if it's public
  ...
  return (
    <div className="home-shell bg-[var(--background)] min-h-screen">
      {/* ── Floating Navbar ──────────────────────────────────────────── */}
      ...
      {/* ── Main Content Area ─────────────────────────────────────────── */}
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-10 pb-12">
        ...
        {/* POPULAR CATEGORIES */}
        <section>
          <SectionHeader title="Popular Categories" href={ROUTES.LOGIN} linkLabel="View all categories" />
        ...
        <section>
          <SectionHeader title="Top Rated Professionals" href={ROUTES.LOGIN} linkLabel="View all providers" />
          ...
          {loadingData ? (
            ...
            <Empty text="No professionals available yet" />
          ) : (
            <motion.div
              variants={staggerContainer}
              initial="hidden"
        ...
        {/* HOW IT WORKS */}
        <section>
          <SectionHeader title="How AllServe works" href={ROUTES.LOGIN} linkLabel="Learn more" />
      ...
      {/* ── Footer ──────────────────────────────────────────────────── */}
      <footer className="border-t border-slate-100/60 py-6 text-center bg-white/50 backdrop-blur-sm mt-auto">
