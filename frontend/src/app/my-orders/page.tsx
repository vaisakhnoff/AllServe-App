...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  awaiting_payment: { label: "Invoice pending", color: "bg-blue-50 text-blue-700" },
...
export default function MyOrdersPage() {
  ...
  const [customerChoiceOrder, setCustomerChoiceOrder] = useState<ServiceOrder | null>(null);
  ...
  useEffect(() => { if (canAccess) fetchOrders(); }, [canAccess, filter, page]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleCustomerChoice = async (orderId: string, choice: "reroute" | "refund") => {
    try {
      if (choice === "refund") {
        await orderService.customerChoice(orderId, { choice: "refund" });
        toast.success("Refund initiated");
      } else {
        toast("Redirecting to provider selection...");
        // In a full implementation this would open a provider picker
        // For now show a placeholder
        await orderService.customerChoice(orderId, { choice: "refund" });
        toast.success("Refund initiated (reroute coming soon)");
      }
      setCustomerChoiceOrder(null);
      fetchOrders();
    } catch (err) {
      toast.error(getErrorMessage(err) || "Failed to process choice");
    }
  };

  if (isInitialized && !canAccess) {
    return <LoginRequiredPrompt title="Login to view orders" message="Sign in to see your service orders." />;
  }
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
            ...
            <Link href="/dashboard" className="btn btn-primary mt-5 px-5 py-2.5 text-sm">
              Browse Services
            </Link>
          </div>
        ) : (
          <div className="space-y-4">
            {orders.map((order) => {
              ...
              return (
                <article key={order._id} className={`premium-card overflow-hidden ${needsChoice ? "ring-2 ring-amber-200" : ""}`}>
                  <div className="p-5">
                    {/* Header */}
                    ...
                    {needsChoice && (
                      <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                        <p className="text-sm font-bold text-amber-800 mb-3">
                          {order.status === "provider_unresponsive"
                            ? "Provider didn't respond in time"
                            : "Provider rejected your request"}
                        </p>
                        <div className="flex gap-3">
                          <button
                            onClick={() => handleCustomerChoice(order._id, "reroute")}
                            className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700"
                          >
                            <RefreshCw size={14} /> Try another provider
                          </button>
                          <button
                            onClick={() => handleCustomerChoice(order._id, "refund")}
