...
- [ ] Define all status enums per delivery model (DirectStatus, InspectionStatus, CustomStatus)
- [ ] Fields: orderId, customerId, providerId, serviceId, categoryId, deliveryModel, subMode, status, statusHistory, description, images, address, exactLocation, preferredDate, preferredTime, responseDeadline, respondedAt, platformFee, platformFeeStatus, budget, budgetType, quoteCount, selectedQuotationId, invoiceId, reviewId, expiresAt, customerChoice, reroutedFromOrderId
- [ ] Add indexes: customerId+status, providerId+status, deliveryModel+status, expiresAt (TTL-like), responseDeadline
- [ ] Create DTOs for each creation endpoint (createDirectInstantSchema, createDirectScheduledSchema, createInspectionSchema, createCustomSchema)

## Task 7: Direct Request Service (Instant + Scheduled)
- [ ] Create `IDirectRequestService` interface
- [ ] Implement `DirectRequestService`:
...
- [ ] Create `DirectRequestController` with endpoints:
  - `POST /orders/direct/instant`
  - `POST /orders/direct/scheduled`
  - `PATCH /orders/:id/accept`
  - `PATCH /orders/:id/reject`
  - `PATCH /orders/:id/customer-choice`
- [ ] Register in DI and router

## Task 8: 30-Minute Timer for Instant Requests
- [ ] Implement timer check mechanism (options: cron job every minute, or MongoDB TTL index + change stream)
- [ ] On timeout: transition order to "provider_unresponsive", trigger customer choice flow
- [ ] Store `responseDeadline` on order creation
- [ ] Create a scheduled job or service that checks expired deadlines
- [ ] Ensure timer is halted (field cleared or ignored) when provider responds

## Task 9: Inspection Request Service
- [ ] Create `IInspectionRequestService` interface
- [ ] Implement `InspectionRequestService`:
...
- [ ] Create `InspectionRequestController` with `POST /orders/inspection`
- [ ] Register in DI and router

## Task 10: Custom Request Service (Broadcast)
- [ ] Create `ICustomRequestService` interface
- [ ] Implement `CustomRequestService`:
...
- [ ] Create `CustomRequestController` with `POST /orders/custom`
- [ ] Implement expiry check (cron or TTL) to transition to "expired"
- [ ] Register in DI and router

## Task 11: Quotation Model & Service
- [ ] Create `Quotation` model (`backend/src/models/quotation.model.ts`)
- [ ] Fields: orderId, providerId, status, currentRevision (labour, material, additional, duration, advance, notes, terms, attachments), revisionHistory[], modificationComment, totalAmount
- [ ] Create Quotation DTOs (createQuotationSchema, reviseQuotationSchema, modificationRequestSchema)
- [ ] Create `IQuotationRepository` interface and implementation
- [ ] Create `IQuotationService` interface
...
- [ ] Create `QuotationController` with all quotation endpoints
- [ ] Register in DI and router

## Task 12: Invoice Model & Service
- [ ] Create `Invoice` model (`backend/src/models/invoice.model.ts`)
- [ ] Fields: orderId, providerId, customerId, labourCharge, materialCost, additionalCharges, discount, total, lineItemNotes, overallRemark, paymentStatus, settledAt, settledBy, settlementMethod, platformCommission
- [ ] Create Invoice DTOs (createInvoiceSchema, markCashSchema)
- [ ] Create `IInvoiceRepository` interface and implementation
...
- [ ] Create `InvoiceController` with `POST /invoices`, `PATCH /invoices/:id/pay-online`, `PATCH /invoices/:id/mark-cash`
- [ ] Register in DI and router

## Task 13: Shared Order Query APIs
- [ ] Create order query DTOs (customerOrderQuerySchema, providerOrderQuerySchema, adminOrderQuerySchema)
- [ ] Create `IServiceOrderRepository` interface with `findByCustomer()`, `findByProvider()`, `findAll()`, `findById()`
- [ ] Implement `ServiceOrderRepository`
- [ ] Create `ServiceOrderController` with `GET /orders`, `GET /orders/:id`, `PATCH /orders/:id/cancel`
- [ ] Implement cancel logic per delivery model (refund rules differ)
- [ ] Register in DI and router

## Task 14: DI Container & Route Registration
- [ ] Update `di.ts` to register all new services and repositories
- [ ] Create new route files or update existing:
  - `/routes/provider-schedule.routes.ts`
  - `/routes/provider-leave.routes.ts`
  - `/routes/provider-status.routes.ts`
  - `/routes/orders.routes.ts`
  - `/routes/quotations.routes.ts`
  - `/routes/invoices.routes.ts`
- [ ] Mount all new routes in `app.ts`
- [ ] Ensure auth middleware is applied correctly per role
